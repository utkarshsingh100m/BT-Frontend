...
if (contactForm) {
    contactForm.addEventListener('submit', function (e) {
        ...
        if (errors.length > 0) {
            showFormNotification(errors.join('<br>'), 'error');
            return;
        }

        // Submit form
        submitContactForm(name, email, message, this);
    });
}
...
// Submit contact form
function submitContactForm(name, email, message, form) {
    ...
    setTimeout(() => {
        // Success simulation
        showFormNotification('Message sent successfully! We\'ll get back to you soon.', 'success');
        form.reset();
        ...
        .catch(error => {
            showFormNotification('Error sending message. Please try again.', 'error');
        })
        .finally(() => {
            submitBtn.disabled = false;
            submitBtn.textContent = originalText;
